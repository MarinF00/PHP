...
class UploadManager
{
    public function uploadImg() {
        ...
        $file_name = $timestamp . "_" . basename($_FILES["thumbnail"]["name"]); // 121321412_duck.jpg
        ...
        $check = getimagesize($_FILES["thumbnail"]["tmp_name"]); // [dsadasdsadsas] | false
        // Is it an image
        if (!$check) {
            $uploadOk = false;
        ...
        if (!$uploadOk) {
            ...
            if (move_uploaded_file($_FILES["thumbnail"]["tmp_name"], $final_path_to_file)) {
                return $file_name;
            } else {
                return false;
            }
